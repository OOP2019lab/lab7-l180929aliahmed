...
//	*/
	...
	transactions[0]=-5000;
	transactions[1]=5000;
	dyArr[0]=new bankAccount(50, 2000, transactions, 2);
...
//	*/
	int accounts=0;
	bankAccount** BankAccounts=readFile("data.txt", accounts);
...
//	*/
	printAccounts(BankAccounts,accounts);
//	
//
//	/********************************TASK 8*************************************
...
//	*/
	makeTransaction(BankAccounts, accounts, 5, 6000, "withdraw");
//
//
//	/********************************TASK 9*************************************
...
//	*/
	deleteAccounts(BankAccounts, accounts);
...
bankAccount** readFile(string file, int& accounts){
	...
	if(fin.fail())
		...
		int id, noOftrans;
		float balance, limit;
		float* transactions=nullptr;
		int t;
		fin>>t;
		accounts=t;
		temp=new bankAccount*[accounts];
		for(int i=0; i<accounts; i++){
			fin>>id;
			fin>>balance;
			fin>>limit;
			fin>>noOftrans;
			transactions= new float[noOftrans];
			for(int j=0; j<noOftrans; j++){
				fin>>transactions[j];
			}
			temp[i]=new bankAccount(id, balance, transactions, noOftrans, limit);
			delete transactions;
	...
	for(int i=0; i<accounts; i++){
		if(B[i]->getID()==id){
			...
			if(transtype=="withdraw"){
				if(B[i]->withdraw(amount))
					return true;
				else
					return false;
			}
			if(transtype=="deposit"){
				if(B[i]->deposit(amount))
					return true;
				else
					return false;
			}
...
void deleteAccounts(bankAccount**& B, int& accounts){
	int count=0;
	for(int i=0; i<accounts; i++){
		if(B[i]->getBalance()!=0 && B[i]->getNoOftrans()!=0)
			count++;
	}
	int index=0;
